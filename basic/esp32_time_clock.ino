...
// LCD配置
LiquidCrystal_I2C lcd(0x27, 16, 2);  // 常见的是16x2
...
void setup() {
  ...
  delay(1000);
  Serial.println("================================");
  Serial.println("ESP32 2行LCD天气显示系统");
  Serial.println("================================");
...
void displayWeatherDetail() {
  ...
  if (weather.valid) {
    lcd.setCursor(0, 1);
    String line = String(weather.temperature, 1) + "C " + String((int)weather.humidity) + "% " + String(weather.windSpeed, 1);
    if (line.length() > 16) {
      line = line.substring(0, 16);
    }
    lcd.print(line);
...
void checkSerialCommands() {
  if (Serial.available()) {
    ...
    if (command == "help") {
      Serial.println("可用命令: help, status, weather, restart, wifi");
